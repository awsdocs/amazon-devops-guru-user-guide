...
| --- | --- | --- | 
|  [Service\-linked role permissions for DevOps Guru](using-service-linked-roles.md#slr-permissions) – Update to an existing policy\.  |  The `AmazonDevOpsGuruServiceRolePolicy` service\-linked role now contains new conditions on the `ssm:CreateOpsItem` and `ssm:AddTagsToResource` actions\.  | June 14, 2021 | 
